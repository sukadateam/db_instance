...
# vs code, var text overlay
gen = None
'''The progress of the key generation. Used to determine the next number in the key.'''
altPassword = None
'''Alternates between using luckyNumber, and the length of the password right before deciding the new key value, per [gen] iteration.'''
#Grouped Files
...
class db_Handler:
    ...
    class Encryption:
        ...
        def uniqueIDGen(self, randomListSize=15, maxKeyLength=200, luckyNumber=None, password=None, consistantOutput=False):
            ...
            \n
            \nArgs:
            \n- randomListSize(int): Each character in the key, is generated by a list of random numbers that has tons of math done to it. 
            \n   -  This is the size of that list.
            \n   -  If consistantOutput is True, this value is ignored.
            ...
            '''
            global gen, alphabet, specialChars, altPassword
            gen = None
            # Check arguments:
            ...
            exCall = '\n\nCall Function: --> Encryption.uniqueIDGen()\n'  # Exception Call
            intList = ['maxKeyLength', 'randomListSize']  # Removed luckyNumber from intList
            for item in intList:
                try:
                    if not isinstance(locals()[item], int):
                        raise Exception(exCall + 'Invalid Argument Type({}), must be int.'.format(str(item)))
                except:
                    raise Exception(exCall + '!Uncalled Exception! Invalid Argument Type({}), must be int.'.format(str(item)))
            ...
            # Switches after each iteration. Used to add password length to the key generation algorithm
            altPassword = True
            # Switched once every 2 iterations and it's only hit when altPassword is False.
            altAfterAlt = False
            ...
            for gen in range(mxSize):
                while True:
                    ...
                    if not consistantOutput:
                        # Generate (randomListSize) random numbers into a list:
                        rList = []
                        ...
                        if password:
                            if altPassword:
                                try:
                                    indx = specialChars.index(password[gen % len(password)])
                                except:
                                    try:
                                        indx = alphabet.index(password[gen % len(password)])
                                    except:
                                        raise Exception(exCall + 'Invalid Character in Password')
                                # If (length of password + indx) together is greater than 9, use the last digit of the (length of the password + indx)
                                if (len(password)+indx) > 9:
                                    length = len(password)
                                    # Instead of len(password). Convert length to a string and use the last digit
                                    length = int(str(length+indx)[-1])
                                    new_key = random.randint(0, length)
                                else:
                                    new_key = random.randint(0, len(password))
